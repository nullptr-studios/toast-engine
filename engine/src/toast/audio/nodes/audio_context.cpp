...
namespace toast {
void AudioContext::addBank(assets::AssetHandle<assets::AudioBank> bank) {
	ZoneScoped;
...
void AudioContext::removeBank(assets::AssetHandle<assets::AudioBank> bank) {
	...
	if (auto it = std::ranges::find_if(m_loaded_banks, [bank](const auto& v) { return v.first == bank; }); it != m_loaded_banks.end()) {
