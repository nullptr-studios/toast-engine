...
void toast::AudioListener::lateTick() {
	...
	if (pos == prev_position && rot == prev_rotation) return; // no need to update the fmod position
	
	glm::vec3 forward_vec = forward();
	glm::vec3 up_vec = up();
	std::optional<glm::vec3> attenuation = attenuation_override.exists() ? std::optional{attenuation_override->pos()} : std::nullopt;
