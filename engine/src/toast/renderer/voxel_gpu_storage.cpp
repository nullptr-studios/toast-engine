...
VoxelSceneUpload::VoxelSceneUpload(
    std::shared_ptr<VoxelGpuStorage> storage, voxel::gpu::PackedPool pool, voxel::gpu::PackedScene scene
)
	...
	const std::array<std::pair<const void*, vk::DeviceSize>, VoxelGpuStorage::k_section_count> sources {
	  {
	   ...
	   {m_scene.records.data(), m_scene.records.size() * sizeof(voxel::gpu::VolumeRecord)},
	   }
	};
