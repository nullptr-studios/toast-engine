...
ultralight::String ToastFontLoader::fallback_font_for_characters(const ultralight::String& /*characters*/, int weight, bool italic) const {
	TOAST_TRACE("[FontLoader] fallback_font_for_characters() called, weight={} italic={}", weight, italic);
	return fallback_font();
}

ultralight::RefPtr<ultralight::FontFile> ToastFontLoader::Load(const ultralight::String& family, int weight, bool italic) {
	...
	if (family_str == "Arial" || family_str == "sans-serif") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "arialbi.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "arialbd.ttf";
		} else if (italic) {
			font_path = fonts_dir + "ariali.ttf";
		} else {
			font_path = fonts_dir + "arial.ttf";
		}
	} else if (family_str == "Times New Roman" || family_str == "serif") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "timesbi.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "timesbd.ttf";
		} else if (italic) {
			font_path = fonts_dir + "timesi.ttf";
		} else {
			font_path = fonts_dir + "times.ttf";
		}
	} else if (family_str == "Courier New" || family_str == "monospace") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "courbi.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "courbd.ttf";
		} else if (italic) {
			font_path = fonts_dir + "couri.ttf";
		} else {
			font_path = fonts_dir + "cour.ttf";
		}
	} else if (family_str == "Segoe UI") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "segoeuiz.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "segoeuib.ttf";
		} else if (italic) {
			font_path = fonts_dir + "segoeuii.ttf";
		} else {
			font_path = fonts_dir + "segoeui.ttf";
		}
	} else if (family_str == "Tahoma") {
		if (weight >= 700) {
			font_path = fonts_dir + "tahomabd.ttf";
		} else {
			font_path = fonts_dir + "tahoma.ttf";
		}
	} else if (family_str == "Verdana") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "verdanaz.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "verdanab.ttf";
		} else if (italic) {
			font_path = fonts_dir + "verdanai.ttf";
		} else {
			font_path = fonts_dir + "verdana.ttf";
		}
	} else if (family_str == "Georgia") {
		if (weight >= 700 && italic) {
			font_path = fonts_dir + "georgiaz.ttf";
		} else if (weight >= 700) {
			font_path = fonts_dir + "georgiab.ttf";
		} else if (italic) {
			font_path = fonts_dir + "georgiai.ttf";
		} else {
			font_path = fonts_dir + "georgia.ttf";
		}
	} else {
