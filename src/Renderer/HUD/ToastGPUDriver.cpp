...
ToastGPUDriver::ToastGPUDriver(ToastGPUContext* context) : context_(context) {
...
void ToastGPUDriver::CreateTexture(uint32_t texture_id, ultralight::RefPtr<ultralight::Bitmap> bitmap) {
	...
	if (bitmap->format() == ultralight::BitmapFormat::A8_UNORM) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, bitmap->width(), bitmap->height(), 0, GL_RED, GL_UNSIGNED_BYTE, pixels);
		bitmap->UnlockPixels();
	...
	if (!bitmap->IsEmpty()) {
		if (bitmap->format() == ultralight::BitmapFormat::A8_UNORM) {
			...
			if (needs_realloc) {
				...
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, bitmap->width(), bitmap->height(), GL_RED, GL_UNSIGNED_BYTE, pixels);
			}
			bitmap->UnlockPixels();
			...
			if (needs_realloc) {
				...
				// Same size - use subimage for efficiency
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, bitmap->width(), bitmap->height(), GL_BGRA, GL_UNSIGNED_BYTE, pixels);
			}
			bitmap->UnlockPixels();
...
void ToastGPUDriver::UpdateGeometry(uint32_t geometry_id, const ultralight::VertexBuffer& vertices, const ultralight::IndexBuffer& indices) {
	PROFILE_ZONE_C(tracy::Color::Orange);
...
void ToastGPUDriver::LoadProgram(ProgramType type) {
	...
	if (type == ultralight::ShaderType::Fill) {
		prog.vert_shader_id =
		    LoadShaderFromSource(GL_VERTEX_SHADER, ultralight::shaders::shader_v2f_c4f_t2f_t2f_d28f_vert().c_str(), "shader_v2f_c4f_t2f_t2f_d28f.vert");
		prog.frag_shader_id = LoadShaderFromSource(GL_FRAGMENT_SHADER, ultralight::shaders::shader_fill_frag().c_str(), "shader_fill.frag");
	} else if (type == ultralight::ShaderType::FillPath) {
		prog.vert_shader_id = LoadShaderFromSource(GL_VERTEX_SHADER, ultralight::shaders::shader_v2f_c4f_t2f_vert().c_str(), "shader_v2f_c4f_t2f.vert");
		prog.frag_shader_id = LoadShaderFromSource(GL_FRAGMENT_SHADER, ultralight::shaders::shader_fill_path_frag().c_str(), "shader_fill_path.frag");
	}
...
void ToastGPUDriver::UpdateUniforms(const ultralight::GPUState& state) {
	...
	ultralight::Matrix model_view_projection =
...
void ToastGPUDriver::SetUniform4f(const char* name, const float val[4]) {
	glUniform4f(
	    ...
	    static_cast<GLfloat>(val[0]),
	    static_cast<GLfloat>(val[1]),
	    static_cast<GLfloat>(val[2]),
	    static_cast<GLfloat>(val[3])
	);
}
...
void ToastGPUDriver::SetUniformMatrix4fv(const char* name, size_t count, const float* val) {
	glUniformMatrix4fv(glGetUniformLocation(cur_program_id_, name), static_cast<GLsizei>(count), GL_FALSE, val);
}
...
ultralight::Matrix ToastGPUDriver::ApplyProjection(const ultralight::Matrix4x4& transform, float screen_width, float screen_height, bool flip_y) {
