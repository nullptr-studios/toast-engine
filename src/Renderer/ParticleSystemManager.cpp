...
// helper to query OpenGL buffer size
static GLint QueryBufferSize(GLuint buf) {
    if (buf == 0) return 0;
    GLint sz = 0;
    glGetNamedBufferParameteriv(buf, GL_BUFFER_SIZE, &sz);
    return sz;
}

// Frame params in std140 layout: ensure allocation size matches the struct
...
void ParticleSystemManager::OnAttach() {
	...
	// FrameParams UBO (std140 binding = 4) - allocate using the std140 struct size
	glCreateBuffers(1, &m_uboFrameParams);
	glNamedBufferStorage(m_uboFrameParams, static_cast<GLsizeiptr>(sizeof(FrameParamsStd140)), nullptr, GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT);
...
void ParticleSystemManager::Tick(float dt) {
	...
	if (spawnWriteTotal > 0) {
		...
		GLint outBufSize = QueryBufferSize(m_ssboOut);
		if (outBufSize <= 0 || writeSize > outBufSize) {
			TOAST_ERROR("SSBO out buffer too small or unallocated (needed %lld, have %d). Skipping spawn write.", (long long)writeSize, outBufSize);
		} else {
			...
			if (ptr) {
				...
				GLint countersBufSize = QueryBufferSize(m_counters);
				if (countersBufSize <= 0 || static_cast<GLsizeiptr>(sizeof(countersVals)) > countersBufSize) {
					TOAST_ERROR("Counters buffer size invalid (needed %zu, have %d)", sizeof(countersVals), countersBufSize);
				} else {
...
void ParticleSystemManager::Render(const glm::mat4& viewProj) {
	...
	GLint uboSize = QueryBufferSize(m_uboFrameParams);
	if (uboSize <= 0 || static_cast<GLsizeiptr>(sizeof(fp)) > uboSize) {
		TOAST_ERROR("FrameParams UBO size invalid (needed %zu, have %d). Skipping compute.", sizeof(fp), uboSize);
	} else {
		...
		if (m_readbackCounter == 0) {
			...
			GLint countersBufSize = QueryBufferSize(m_counters);
			if (countersBufSize <= 0 || static_cast<GLsizeiptr>(sizeof(countersRead)) > countersBufSize) {
				TOAST_ERROR("Counters buffer size invalid for readback (needed %zu, have %d)", sizeof(countersRead), countersBufSize);
			} else {
