...
std::optional<RayResult> RayCast(const vec2 point, const vec2 dir, ColliderFlags flags) {
	...
	Line ray = { .p1 = point, .p2 = point + normalize(dir) * std::numeric_limits<float>::max(), .normal = normal, .tangent = direction };

	return PhysicsSystem::RayCollision(&ray, flags);
}

toast::Node* PointCast(const vec2 point, ColliderFlags flags) {
