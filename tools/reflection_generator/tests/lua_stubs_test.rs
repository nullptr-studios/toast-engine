...
"#;
    ...
    assert!(
        stubs.contains("---@class StubNode"),
        "class missing:\n{stubs}"
    );
    assert!(
        stubs.contains("---@field health integer"),
        "m_ prefix should be stripped:\n{stubs}"
    );
    assert!(
        stubs.contains("---@field tint color3"),
        "[[Color]] vec3 should be color3:\n{stubs}"
    );
    assert!(
        !stubs.contains("secret"),
        "[[Hidden]] fields should be skipped:\n{stubs}"
    );
    assert!(
        stubs.contains("---@field frames Asset[]"),
        "asset arrays should be Asset[]:\n{stubs}"
    );
    assert!(
        stubs.contains("---@field target Node3D"),
        "Box<T> should use the bare node type:\n{stubs}"
    );
    assert!(
        stubs.contains(
    ...
    assert!(
        ...
        "zero-argument signal missing:\n{stubs}"
    );
    assert!(
        stubs.contains("---@field hit Signal2<integer, number>"),
        "typed signal missing:\n{stubs}"
    );
    assert!(
        stubs.contains("StubNode = nil"),
        "type marker global missing:\n{stubs}"
    );
}
