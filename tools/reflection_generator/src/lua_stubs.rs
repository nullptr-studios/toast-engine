...
use crate::{Field, FieldType, Function, Signal};
...
pub fn generate_lua_stubs(nodes: &[NodeInfo]) -> String {
    ...
    out.push_str(
        "-- One class per reflected node type: fields, methods and the global type marker.\n\n",
    );
    ...
    for node in nodes {
        ...
        for signal in &node.class.signals {
            out.push_str(&format!(
                "---@field {} {}\n",
                signal.name,
                signal_lua_type(signal)
            ));
        }
