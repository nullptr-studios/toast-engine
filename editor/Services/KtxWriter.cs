...
internal static class KtxWriter {
	...
	public static async Task ConvertTexture(
		string srcPath, string destPath, TextureImportSettings s, Action<string>? log = null) {
	...
	private static void PrepareImage(string srcPath, string tempPng, TextureImportSettings s, Action<string>? log) {
	...
	private static string BuildArgs(TextureImportSettings s, string destPath, string inputPath) {
		var sb = new StringBuilder("--t2 ");

		if (s.GenerateMipmaps)
			sb.Append("--genmipmap ");

		// BC7/ASTC → UASTC (high quality, transcodes to BC7/ASTC at load time)
		// BC1/3/4/5 → ETC1S/BasisLZ (small, transcodes to the BCn family)
		...
		// Only apply Zstd to UASTC / uncompressed output.
	...
	private static async Task RunToktx(string args, Action<string>? log) {
		...
		// toktx is dynamically linked against the ktx shared library
		// if its missing the process exits 127 with an opaque loader error
