...
public static partial class Events {
	...
	internal static uint CreateListener() => events_listener_create();

	internal static void DestroyListener(uint handle) => events_listener_destroy(handle);

	internal static void ForgetSubscription(ulong id) => Subscribers.TryRemove(id, out _);
	...
	private static unsafe int OnNativeEvent(byte* name, byte* data, uint size, void* userData) {
		try {
			...
			if (!Subscribers.TryGetValue(id, out var subscription)) {
				// unsubscribed already, skip
				return 0;
			}

			var span = new ReadOnlySpan<byte>(data, (int)size);
			var message = subscription.Parser.ParseFrom(span);
			// returning 1 consumes the event
	...
	private static unsafe partial int events_listener_subscribe(uint handle, string name, delegate* unmanaged[Cdecl]<byte*, byte*, uint, void*, int> callback, void* user_data, sbyte priority);
