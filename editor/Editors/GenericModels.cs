...
public partial class GenericFieldVM : ObservableObject, IRowSplittable, IRowVisible, IStructRow {
	...
	IList IStructRow.StructKeys => Children;
	...
	partial void OnChildrenLockedChanged(bool value) {
		OnPropertyChanged(nameof(ChildrenEditable));
	}
	...
	// Dirty propagation
	partial void OnFloatValChanged(float value) {
		NotifyDirty?.Invoke();
	}

	partial void OnIntValChanged(int value) {
		NotifyDirty?.Invoke();
	}

	partial void OnBoolValChanged(bool value) {
		NotifyDirty?.Invoke();
	}

	partial void OnStringValChanged(string value) {
		NotifyDirty?.Invoke();
	}

	partial void OnRefUidChanged(string value) {
		NotifyDirty?.Invoke();
	}
	...
	private static string InferArrayType(TomlArray arr) {
		var allNumbers = arr.Count > 0 && arr.All(e => e is double or float or long or int);
		if (allNumbers)
	...
	private static float TomlToFloat(object? v) {
		return v switch {
			double d => (float)d,
			float f => f,
			long l => l,
	...
	private static int TomlToInt(object? v) {
		return v switch {
			long l => (int)l,
			int i => i,
			double d => (int)d,
