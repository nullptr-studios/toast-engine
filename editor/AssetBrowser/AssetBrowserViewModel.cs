...
public sealed class BreadcrumbItem {
    ...
    public BreadcrumbItem(string label, System.Action navigate, bool isLast) {
        Label    = label;
        IsLast   = isLast;
...
public class AssetBrowserViewModel : ViewModelBase, INotifyPropertyChanged {
    ...
    public bool FilterNode {
        get => m_filterNode;
        set { m_filterNode = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterTexture {
        get => m_filterTexture;
        set { m_filterTexture = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterModel {
        get => m_filterModel;
        set { m_filterModel = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterMaterial {
        get => m_filterMaterial;
        set { m_filterMaterial = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterShader {
        get => m_filterShader;
        set { m_filterShader = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterScript {
        get => m_filterScript;
        set { m_filterScript = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }
    public bool FilterUnknown {
        get => m_filterUnknown;
        set { m_filterUnknown = value; Notify(); Notify(nameof(FilterAll)); Notify(nameof(CurrentItems)); Notify(nameof(ItemCount)); }
    }

    // Tri-state: true = all, false = none, null = mixed
    public bool? FilterAll {
        get {
            bool all  = m_filterNode && m_filterTexture && m_filterModel && m_filterMaterial && m_filterShader && m_filterScript && m_filterUnknown;
            bool none = !m_filterNode && !m_filterTexture && !m_filterModel && !m_filterMaterial && !m_filterShader && !m_filterScript && !m_filterUnknown;
            ...
            if (!string.IsNullOrWhiteSpace(m_searchText)) {
                ...
                return Folders
                    ...
                    .Where(f => string.IsNullOrEmpty(textFilter) ||
                                f.Name.Contains(textFilter, System.StringComparison.OrdinalIgnoreCase))
                    .Cast<object>();
            }
            if (m_selectedFolder is null) return [];
    ...
    private void LoadFolders() {
        ...
        var assetsPath = @"C:\Users\Xein\Desktop\unnamed_project\assets";
        var corePath   = @"C:\Users\Xein\code\toast-engine\out\Debug\toast_engine\bin\assets";
    ...
    private void Refresh() => LoadFolders();
    ...
    private static (string text, FileType? type) ParseSearch(string query) {
        ...
        var text = query;
        if (match.Success) {
            if (System.Enum.TryParse<FileType>(match.Groups[1].Value, ignoreCase: true, out var ft))
                typeFilter = ft;
    ...
    private bool IsTypeVisible(FileType type) => type switch {
